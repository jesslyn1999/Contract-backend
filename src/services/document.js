...
const updateDocsTanggal = (_no, oldTanggal, newTanggal) =>
{
    return new Promise((resolve, reject) =>
    {
        ...
        tagModelInstance.findOneAndUpdate({no_lampiran: _no, tanggal_terbit: oldTanggal}, {no_lampiran: _no, tanggal_terbit: newTanggal}, function(err)
        {
            if (err)
            {
                reject(err);
...
const updateDocsPemenang = (_no, oldPemenang, newPemenang) =>
{
    return new Promise((resolve, reject) =>
    {
        ...
        tagModelInstance.findOneAndUpdate({no_lampiran: _no, nama_pemenang: oldPemenang}, {no_lampiran: _no, nama_pemenang: newPemenang}, function(err)
        {
            if (err)
            {
                reject(err);
...
const updateDocsKontrak = (_no, oldKontrak, newKontrak) =>
{
    return new Promise((resolve, reject) =>
    {
        ...
        tagModelInstance.findOneAndUpdate({no_lampiran: _no, nilai_kontrak: oldKontrak}, {no_lampiran: _no, nilai_kontrak: newKontrak}, function(err)
        {
            if (err)
            {
                reject(err);
