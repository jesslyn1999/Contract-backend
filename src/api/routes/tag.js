...
export default app => {
    ...
    route.post('/', (req, res) => {
        tagService
            ...
            .catch(err => {
                logger.error(
                    `[TagRoute][SetNewTag]: Failed to set new tag`,
                    err,
                );
                return res.status(500).json({
                    request: {
                        success: false,
                        message:
                            'Internal server error, report to admin for assistance ${err}\n',
                    },
                });
            });
    });
    ...
    route.delete('/:key', (req, res) => {
        tagService
            ...
            .catch(err => {
                logger.error(
                    `[TagRoute][DeleteTag]: Failed to delete tag`,
                    err,
                );
                return res.status(500).json({
                    request: {
    ...
    route.get('/updatetag', (req, res) => {
        tagService
            ...
            .catch(err => {
                logger.error(
                    `[TagRoute][UpdateTag]: Failed to update existing tag`,
                    err,
                );
                return res.status(500).json({
                    request: {
